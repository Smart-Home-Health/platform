 ...
 */
// THE intake logging form. One feed is a LIST of items — formula plus a
...
//
// This is only the FORM — IntakeSheet wraps it in an EntityModal for the
// phone flows, and the live dashboard hosts it inline in the docked side
// pane (an .em-inline host), where a centered dialog makes no sense.
...
import {
  feedTarget, makeItemRow, rowIsValid, rowToItemPayload, rowsFromScheduleRow,
  saveRowsAsItems,
} from './intakeItemRows';
...
export default function IntakeForm({
  ...
  useEffect(() => {
    if (!active || !patient || editing) return;
    let cancelled = false;
    Promise.all([
  ...
  useEffect(() => {
    if (!active || !patient || editing || careTaskLogId) return;
    let cancelled = false;
    ...
    return () => { cancelled = true; };
  }, [active, patient, editing, careTaskLogId]);
  ...
  const chooseFeed = (key) => {
    ...
    if (items.length === 0) setItems(rowsFromScheduleRow(feed));
  };
